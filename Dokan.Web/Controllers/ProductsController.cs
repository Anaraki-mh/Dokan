    ...
    public class ProductsController : Controller
    {
        ...
        public async Task<ActionResult> List(int productCategoryId, int minPrice, int maxPrice, bool onlyDeals, int page = 1)
        {
            ...
            if (onlyDeals == true)
                ...
                allEntities = allEntities.Where(x => x.DiscountCategoryId > 0).ToList();

            // If a category is selected...
            if (productCategoryId > 0)
                // Only keep the entities with that category in the list
                allEntities = allEntities.Where(x => x.ProductCategoryId == productCategoryId).ToList();

            // Find the Max and Min prices in the list of products and pass the value to 2 variables
            int absMaxPrice = Convert.ToInt32(Math.Ceiling(allEntities.Select(x => x.Price).Max()));
            int absMinPrice = Convert.ToInt32(Math.Ceiling(allEntities.Select(x => x.Price).Min()));
            ...
            allEntities = allEntities.Where(x => x.Price >= minPrice && x.Price <= maxPrice).ToList();
        ...
        public async Task<ActionResult> Filters()
        {
            ...
            ViewBag.MinPrice = Convert.ToInt32(Math.Ceiling(allEntities.Select(x => x.Price).Min()));
            ViewBag.MaxPrice = Convert.ToInt32(Math.Ceiling(allEntities.Select(x => x.Price).Max()));
        ...
        {
            var allEntities = await _productService.SearchAsync(searchString);
